...
export default function CriarPedidos() {
  ...
  useEffect(() => {
    const carregarMercados = () => {
      /*
       * O cookie JWT é enviado automaticamente pelo navegador.
       */
      api
        .get("/mercados")
        .then((response) => {
        ...
        .then((response) => {
          if (response.data.length == 0) {
            ...
            setProdutosData((prev) => ({
              ...prev,
              pre_lavados: preLavadosFormatados,
            }));
          }
        })
        .catch((error) => {
      ...
      api
        ...
        .then((response) => {
          if (response.data.length == 0) {
            ...
            setProdutosData((prev) => ({
              ...prev,
              nao_lavados: naoLavadosFormatados,
            }));
          }
        })
        .catch((error) => {
    ...
    /*
     * Não verificamos mais se existe token no localStorage.
     * A ProtectedRoute já validou a sessão com o backend.
     */
    carregarMercados();
    carregarPreLavados();
    carregarNaoLavados();
  ...
  const handleLancarPedido = () => {
    ...
    /*
     * O cookie JWT e o token CSRF são enviados pelo cliente api.
     */
    api
  ...
  const renderBadgeEmbalagem = (embalagem) => {
    ...
    switch (embalagem?.toLowerCase()) {
      case "pote":
        return (
          <span className={`${baseClass} bg-[#e6f0ff] text-[#3b82f6]`}>
            Pote
          </span>
        );
      case "bandeja":
        return (
          <span className={`${baseClass} bg-[#ffebe6] text-[#ff5c33]`}>
            Bandeja
          </span>
        );
      case "saco":
        return (
          <span className={`${baseClass} bg-[#e6fcf5] text-[#0ca678]`}>
            Saco
          </span>
        );
      default:
  ...
  const LinhaProduto = ({ produto }) => {
    ...
    const handleBlur = () => {
      if (
        itemNoCarrinho &&
        (itemNoCarrinho.qtd === "" || itemNoCarrinho.qtd === 0)
      ) {
        removerItem(produto.id);
      }
    };
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200/80 p-5 flex items-center sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="w-full sm:w-120 h-9">
              <Select
                options={["Escolha um mercado..."].concat(
                  mercadosData.map((m) => m.nome),
                )}
                value={
                  mercadoSelecionado === "Selecione um mercado"
                    ? "Escolha um mercado..."
                    : mercadoSelecionado
                }
          ...
          <div className="flex flex-col gap-3">
            <SecaoSanfona
              ...
              mobileItems={produtosData.nao_lavados?.map((p) => (
                <CardProduto key={p.id} produto={p} />
              ))}
            >
              {produtosData.nao_lavados?.map((p) => (
            ...
            <SecaoSanfona
              ...
              abaAberta={abaAberta}
              setAbaAberta={setAbaAberta}
              mobileItems={produtosData.pre_lavados?.map((p) => (
                <CardProduto key={p.id} produto={p} />
              ))}
            >
              {produtosData.pre_lavados?.map((p) => (
                <LinhaProduto key={p.id} produto={p} />
              ))}
            </SecaoSanfona>
          </div>
